...
organize_files(){
    ...
    if [ "$nolc" = true ] || [ "$nocc" = true ]
    then
        analyze "$targetFolder/$destinationFolder/$name"
    fi
    if [ "$noexecute" = true ]
    then
        runCode "$targetFolder/$destinationFolder/$name" "$testFolder" "$answerFolder" "$destinationFolder"
    fi
}
runCode(){
    code="$1"
    ...
    case "$extension" in
        py)
            ...
            do
                ...
                python3 "$code" < "$test" > "$destinationFolder/out${id}.txt"
            done
        ;;
        c)
            ...
            do
                testname=$(basename "$test")
                id=$(echo "$testname" | grep -o -E '[0-9]+')
                "$destinationFolder/main.out" < "$test" > "$destinationFolder/out${id}.txt"
            done
        ;;
        cpp)
        ...
        ;;
        java)
            java -cp "$destinationFolder" Main
            if [ $? -ne 0 ]
            then
                echo "Compilation Error"
                return
            ...
            do
                testname=$(basename "$test")
                id=$(echo "$testname" | grep -o -E '[0-9]+')
                java -cp "$destinationFolder" Main < "$test" > "$destinationFolder/out${id}.txt"
